...
public class VirtualStorageImpl implements VirtualStorage, MessageHandler {
	...
	private final Cache<String, Measurement> measurementCache;
	...
	public VirtualStorageImpl(CacheService cacheService) throws Exception {
		...
		this.measurementCache = cacheService.getCache("measurement", String.class, Measurement.class);
	}
	...
	public void messageReceived(Message message) {
		...
		case Measurement: {
			...
			String deviceId = measurementMessage.getDeviceId();
			Measurement measurement = measurementMessage.getMeasurement();
			try {
				this.measurementCache.put(deviceId, measurement);
			}catch(Exception ex) {
				ex.printStackTrace();
			}
			break;
