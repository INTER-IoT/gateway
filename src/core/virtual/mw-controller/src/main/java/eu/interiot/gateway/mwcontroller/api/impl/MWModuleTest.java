...
public class MWModuleTest implements MWModule {
	
	private Logger log = LogManager.getLogger(MWModuleTest.class);
	...
	public void registerDevice(DeviceDefinition device) throws Exception {
		log.info("REGISTERING DEVICE: " + device.getId() + ", " + device.getType());
	}
	...
	public void pushMeasurement(String deviceId, Measurement measurement) throws Exception {
		...
		measurement.getData().forEach(data -> sj.add(data.getAttribute().getName()+"="+data.getValue()));
		log.info("PUSHING MEASUREMENT OF " + deviceId + " " + sj.toString());
	}
}
