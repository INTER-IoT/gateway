...
@Api("virtual")
@Path("/virtual")
public class VirtualApiRouter{
	
	@GET
    @Path("/info")
    @Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Get virtual gateway information", response = String.class)
    public Response info() {
		try {
			return ApiResponse.jsonResponse(200, GatewayInfo.localInstance()).build();
		} catch (Exception e) {
